...
const buildSchedulesFromProfile = (
    profile: DoctorProfile | null,
    ...
    const clinicWorkingDays = clinic?.working_hours ?? [];

    return clinicWorkingDays
        .filter((day) => day.is_active)
        .map((day) => {
            const schedule = schedulesByDay.get(normalizeDayOfWeek(day.day_of_week));
...
const defaultsFor = (profile: DoctorProfile | null): DoctorForm => ({
    ...
    schedules: buildSchedulesFromProfile(profile, clinicForId(profile?.clinic_id)),
...
const markWorkingDayForSync = (dayOfWeek: number): void => {
    if (!form.schedule_days_to_sync.includes(dayOfWeek)) {
        form.schedule_days_to_sync.push(dayOfWeek);
    }
};

const resetSchedulesForSelectedClinic = (): void => {
    form.schedules = (selectedClinic.value?.working_hours ?? [])
        .filter((day) => day.is_active)
        .map((day) => ({
            day_of_week: normalizeDayOfWeek(day.day_of_week),
            is_available: false,
    ...
    form.schedule_days_to_sync = [];
    form.clearErrors('schedules', 'schedule_days_to_sync');
};
...
watch(
    ...
    (clinicId, previousClinicId) => {
        if (
            ...
            clinicId === '' ||
            previousClinicId === undefined ||
            clinicId === previousClinicId
        ) {
            return;
        }

        resetSchedulesForSelectedClinic();
    },
    { flush: 'sync' },
);
            ...
            >
                <section class="space-y-3">
                    ...
                    <div class="grid gap-4 md:grid-cols-2">
                        ...
                        <div class="grid gap-2">
                            <Label for="doctor_employment_start_date"
                            ...
                            />
                            <InputError
                                :message="form.errors.employment_start_date"
                            />
                        </div>
                ...
                <DoctorWorkingHoursSelector
                    ...
                    :clinic-working-hours="selectedClinic?.working_hours ?? []"
                    @changed="markWorkingDayForSync"
                />
