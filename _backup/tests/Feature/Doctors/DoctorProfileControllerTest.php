...
class DoctorProfileControllerTest extends TestCase
{
    ...
    public function test_doctors_and_clinics_pages_are_restricted_to_administrators(): void
    {
        $clinic = Clinic::factory()->create();
        $this->authenticateForClinic($clinic, 'doctor');

        $this->getJson(route('doctors.index'))->assertForbidden();
        $this->getJson(route('clinics.index'))->assertForbidden();
    }

    public function test_index_eager_loads_the_doctor_clinic_and_schedules(): void
    {
        $clinic = Clinic::factory()->create();
        $this->authenticateForClinic($clinic);
        $doctor = $this->createDoctorUser($clinic);
        ...
        $response->assertOk();
        $response->assertJsonPath('data.0.id', $profile->id);
        $response->assertJsonPath('data.0.clinic.id', $clinic->id);
        $response->assertJsonPath('data.0.schedules.0.start_time', '09:00');
        $response->assertJsonPath('data.0.schedules.0.end_time', '13:00');
    }

    public function test_deleting_one_doctor_keeps_the_shared_clinic_and_other_doctors_intact(): void
    {
        $clinic = Clinic::factory()->create();
        $this->authenticateForClinic($clinic);
        $firstDoctor = $this->createDoctorUser($clinic);
        $secondDoctor = $this->createDoctorUser($clinic);
        $firstProfile = DoctorProfile::factory()->create([
            'clinic_id' => $clinic->id,
            'user_id' => $firstDoctor->id,
        ]);
        $secondProfile = DoctorProfile::factory()->create([
            'clinic_id' => $clinic->id,
            'user_id' => $secondDoctor->id,
        ]);

        $this->deleteJson(route('doctors.destroy', ['doctorProfileId' => $firstProfile->id]))
            ->assertNoContent();

        $this->assertSoftDeleted($firstProfile);
        $this->assertModelExists($clinic);
        $this->assertModelExists($secondProfile);
    ...
    public function test_index_passes_clinic_options_without_schedules(): void
    {
        $clinic = Clinic::factory()->create();
        $this->authenticateForClinic($clinic);
    ...
    public function test_store_creates_doctor_profile_with_audit_log(): void
    {
        ...
        $payload = [
            ...
            'phone' => '+963999000111',
            'employment_start_date' => '2026-06-14',
            ...
            'schedules' => [
                ...
                ['day_of_week' => 0, 'is_available' => true, 'start_time' => '09:00', 'end_time' => '13:00'],
                ['day_of_week' => 1, 'is_available' => true, 'start_time' => '10:00', 'end_time' => '14:00'],
                ['day_of_week' => 2, 'is_available' => false, 'start_time' => null, 'end_time' => null],
                ['day_of_week' => 3, 'is_available' => false, 'start_time' => null, 'end_time' => null],
                ['day_of_week' => 4, 'is_available' => false, 'start_time' => null, 'end_time' => null],
                ['day_of_week' => 5, 'is_available' => false, 'start_time' => null, 'end_time' => null],
            ],
        ...
        $response->assertJsonPath('data.employment_start_date', '2026-06-14');
    ...
    public function test_store_rejects_doctor_hours_outside_clinic_schedules(): void
    {
        $clinic = Clinic::factory()->create();
        $this->authenticateForClinic($clinic);
        ...
        $response = $this->postJson(route('doctors.store'), [
            ...
            'schedules' => [
                ...
                ['day_of_week' => 2, 'is_available' => true, 'start_time' => '10:00', 'end_time' => '12:00'],
            ],
        ]);
    ...
    public function test_store_rejects_user_without_doctor_role(): void
    {
        ...
        $response = $this->postJson(route('doctors.store'), [
            ...
            'schedules' => [
                ...
                ['day_of_week' => 1, 'is_available' => false, 'start_time' => null, 'end_time' => null],
                ['day_of_week' => 2, 'is_available' => false, 'start_time' => null, 'end_time' => null],
                ['day_of_week' => 3, 'is_available' => false, 'start_time' => null, 'end_time' => null],
                ['day_of_week' => 4, 'is_available' => false, 'start_time' => null, 'end_time' => null],
                ['day_of_week' => 5, 'is_available' => false, 'start_time' => null, 'end_time' => null],
    ...
    public function test_show_returns_clinic_days_and_schedules_using_numeric_weekdays(): void
    {
        $clinic = Clinic::factory()->create();
        $this->authenticateForClinic($clinic);
        ...
        $indexResponse->assertJsonPath('data.0.schedules.0.day_of_week', 3);
        $indexResponse->assertJsonPath('data.0.schedules.0.start_time', '11:00');
        $indexResponse->assertJsonPath('data.0.schedules.0.is_available', true);
        $indexResponse->assertJsonPath('data.0.schedules.0.start_time', '11:00');

        $response = $this->getJson(route('doctors.show', ['doctorProfileId' => $profile->id]));

        $response->assertOk();
        $response->assertJsonPath('data.schedules.0.day_of_week', 3);
        $response->assertJsonPath('data.schedules.0.start_time', '11:00');
        $response->assertJsonPath('data.schedules.0.day_of_week', 3);
        $response->assertJsonPath('data.schedules.0.start_time', '11:00');
        $response->assertJsonPath('data.schedules.0.is_available', true);
        $response->assertJsonPath('data.schedules.0.end_time', '17:00');
        $response->assertJsonMissingPath('data.doctor_schedules');
    }
    ...
    public function test_update_updates_doctor_profile_and_writes_audit_log(): void
    {
        ...
        $response = $this->putJson(route('doctors.update', ['doctorProfileId' => $profile->id]), [
            ...
            'schedules' => [
                ['day_of_week' => 6, 'is_available' => true, 'start_time' => '09:00', 'end_time' => '17:00'],
                ['day_of_week' => 0, 'is_available' => false, 'start_time' => null, 'end_time' => null],
                ['day_of_week' => 1, 'is_available' => false, 'start_time' => null, 'end_time' => null],
                ['day_of_week' => 2, 'is_available' => false, 'start_time' => null, 'end_time' => null],
                ['day_of_week' => 3, 'is_available' => false, 'start_time' => null, 'end_time' => null],
        ...
        $response->assertJsonPath('data.employment_start_date', '2026-07-01');
    ...
    public function test_update_rejects_doctor_hours_outside_clinic_schedules(): void
    {
        $clinic = Clinic::factory()->create();
        $this->authenticateForClinic($clinic);
        ...
        $response = $this->putJson(route('doctors.update', ['doctorProfileId' => $profile->id]), [
            'clinic_id' => $clinic->id,
            'schedules' => [
                ['day_of_week' => 0, 'is_available' => true, 'start_time' => '07:00', 'end_time' => '13:00'],
            ],
        ]);

        $response->assertUnprocessable();
        $response->assertJsonValidationErrors(['schedules.0.start_time']);
    }

    public function test_update_replaces_schedules_with_only_active_days(): void
    {
        $clinic = Clinic::factory()->create();
        $this->authenticateForClinic($clinic);
        ...
        $response = $this->putJson(route('doctors.update', ['doctorProfileId' => $profile->id]), [
            ...
            'schedules' => [
                ['day_of_week' => 2, 'is_available' => false, 'start_time' => null, 'end_time' => null],
                ['day_of_week' => 3, 'is_available' => true, 'start_time' => '11:00', 'end_time' => '17:00'],
            ],
        ]);
    ...
    public function test_update_preserves_unedited_doctor_schedule_days(): void
    {
        $clinic = Clinic::factory()->create();
        $this->authenticateForClinic($clinic);
        ...
        $response = $this->putJson(route('doctors.update', ['doctorProfileId' => $profile->id]), [
            'clinic_id' => $clinic->id,
            'schedules' => [
                ['day_of_week' => 2, 'is_available' => true, 'start_time' => '10:00', 'end_time' => '15:00'],
                ['day_of_week' => 3, 'is_available' => true, 'start_time' => '13:00', 'end_time' => '16:00'],
            ],
            'schedule_days_to_sync' => [2],
        ]);
    ...
    public function test_update_keeps_schedules_when_the_form_did_not_edit_schedules(): void
    {
        $clinic = Clinic::factory()->create();
        $this->authenticateForClinic($clinic);
