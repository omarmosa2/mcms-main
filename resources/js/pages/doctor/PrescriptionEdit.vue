...
const { prescription, patients } = defineProps<{
...
defineOptions({
    layout: {
        breadcrumbs: [
            ...
            { title: prescription.prescription_number, href: `/doctor/prescriptions/${prescription.id}` },
            { title: 'تعديل', href: `/doctor/prescriptions/${prescription.id}/edit` },
        ],
    },
});
...
function submit() {
    form.put(`/doctor/prescriptions/${prescription.id}`);
}
</script>

<template>
    <Head :title="`تعديل ${prescription.prescription_number}`" />

    <div class="container-modern space-y-6 py-6" dir="rtl">
        <!-- Header -->
        <section class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div class="flex items-center gap-3">
                <Link :href="`/doctor/prescriptions/${prescription.id}`" class="inline-flex items-center gap-1 text-sm text-muted-foreground hover:text-foreground">
                    <ArrowLeft class="size-4" />
                    العودة
                </Link>
                <div class="flex items-center gap-3">
                    <div class="flex size-10 items-center justify-center rounded-xl bg-amber-100">
                        <FileText class="size-5 text-amber-700" />
                    </div>
                    <div>
                        <h1 class="text-xl font-bold text-[#111827]">تعديل وصفة طبية</h1>
                        <p class="text-sm text-muted-foreground">{{ prescription.prescription_number }}</p>
                    </div>
                </div>
            </div>
        </section>
        ...
        <form @submit.prevent="submit" class="space-y-6">
            ...
            <div class="rounded-2xl border border-[#E2ECF6] bg-white p-5 space-y-5">
                <div class="flex items-center justify-between border-b border-border/50 pb-3">
                    <h3 class="text-sm font-semibold text-slate-900">الأدوية ({{ form.items.length }})</h3>
            ...
            <div class="flex items-center justify-end gap-3">
                <Link :href="`/doctor/prescriptions/${prescription.id}`" class="inline-flex items-center gap-2 rounded-xl border border-border/60 px-4 py-2.5 text-sm font-medium transition-colors hover:bg-muted/50">
                    إلغاء
                </Link>
