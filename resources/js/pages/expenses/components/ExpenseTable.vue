...
<template>
    <section class="glass-panel-soft overflow-hidden">
        <div
            ...
            <div
                class="flex items-center gap-2 rounded-xl border border-border/70 bg-secondary/40 px-3 py-2 text-xs text-muted-foreground">
                <span>الصفحة</span>
            ...
            <div class="rounded-2xl border border-border/70 bg-secondary/20 p-4 shadow-sm">
                <div class="grid gap-3 lg:grid-cols-12 lg:items-end">
                    <div class="grid gap-1.5 lg:col-span-4">
                        <Label for="expenses_search"
                            class="flex items-center gap-1 text-xs font-semibold text-foreground">
                            <Search class="size-3.5 text-primary" />
                            بحث
                        </Label>
                        <FilterSearch id="expenses_search" :model-value="localSearch"
                            placeholder="العنوان، الجهة، الرقم المرجعي..." class="h-11 rounded-xl"
                            @update:model-value="(value) => emit('update-search', value)" />
                    </div>

                    <div class="grid gap-3 sm:grid-cols-2 lg:col-span-5 lg:grid-cols-3">
                        <div class="grid gap-1.5">
                            <Label for="expenses_status"
                                class="flex items-center gap-1 text-xs font-semibold text-foreground">
                                <Filter class="size-3.5 text-primary" />
                                الحالة
                            </Label>
                            <FilterSelect id="expenses_status" :model-value="localStatus" :options="statusOptions"
                                placeholder="الكل" class="h-11 rounded-xl"
                                @update:model-value="(value) => emit('update-status', String(value ?? 'all'))" />
                        </div>
                        <div class="grid gap-1.5">
                            <Label for="expenses_category"
                                class="flex items-center gap-1 text-xs font-semibold text-foreground">
                                <Filter class="size-3.5 text-primary" />
                                التصنيف
                            </Label>
                            <FilterSelect id="expenses_category" :model-value="localCategoryId"
                                :options="categoryOptions" placeholder="الكل" class="h-11 rounded-xl"
                                @update:model-value="(value) => emit('update-category-id', value === null ? null : Number(value))" />
                        </div>
                        <div class="grid gap-1.5">
                            <Label for="expenses_clinic"
                                class="flex items-center gap-1 text-xs font-semibold text-foreground">
                                <Filter class="size-3.5 text-primary" />
                                العيادة
                            </Label>
                            <FilterSelect id="expenses_clinic" :model-value="localClinicId" :options="clinicOptions"
                                placeholder="الكل" class="h-11 rounded-xl"
                    ...
                    <div class="grid gap-3 sm:grid-cols-2 lg:col-span-3">
                        <div class="grid gap-1.5">
                            <Label for="expenses_payment_method"
                                class="flex items-center gap-1 text-xs font-semibold text-foreground">
                                <Filter class="size-3.5 text-primary" />
                                طريقة الدفع
                            </Label>
                            <FilterSelect id="expenses_payment_method" :model-value="localPaymentMethod"
                                :options="paymentMethodOptions" placeholder="الكل" class="h-11 rounded-xl"
                        ...
                        <div class="grid gap-1.5">
                            ...
                            <select id="expenses_per_page" :value="localRowsPerPage"
                ...
                <div class="mt-3 grid gap-3 lg:grid-cols-[minmax(0,1fr)_auto] lg:items-end">
                    <div class="grid gap-1.5">
                        ...
                        <FilterDateRange :from="localDateFrom" :to="localDateTo"
                            @update:from="(value) => emit('update-date-from', value)"
                            @update:to="(value) => emit('update-date-to', value)" />
                    </div>
                    <Button type="button" variant="outline" class="h-11 rounded-xl px-4 text-xs"
                        @click="emit('clear-filters')">
                        تصفية جديدة
                    </Button>
                </div>

                <FilterBar v-if="activeFilters.length > 0" class="mt-3" :active-filters="activeFilters"
                    @remove="(key) => emit('remove-filter', key)" @clear-all="emit('clear-filters')" />
            </div>
        </div>

        <div class="rounded-2xl border border-border/70 bg-card shadow-sm mx-5 mb-5">
            <div class="overflow-x-auto">
                <table class="w-full border-separate border-spacing-0">
                    <thead>
                        <tr class="bg-secondary/50">
                            <th
                                class="border-b border-border px-3 py-3 text-center text-[0.72rem] font-bold text-muted-foreground">
                                #</th>
                            <th
                                class="border-b border-border px-3 py-3 text-center text-[0.72rem] font-bold text-muted-foreground">
                                <button type="button"
                            ...
                            <th
                                class="border-b border-border px-3 py-3 text-center text-[0.72rem] font-bold text-muted-foreground">
                                العنوان</th>
                            <!-- <th class="border-b border-border px-3 py-3 text-center text-[0.72rem] font-bold text-muted-foreground">العيادة</th> -->
                            <th
                                class="border-b border-border px-3 py-3 text-center text-[0.72rem] font-bold text-muted-foreground">
                                <button type="button"
                                    class="inline-flex items-center gap-1.5 transition hover:text-foreground"
                            ...
                            <!-- <th class="border-b border-border px-3 py-3 text-center text-[0.72rem] font-bold text-muted-foreground">طريقة الدفع</th> -->
                            <th
                                class="border-b border-border px-3 py-3 text-center text-[0.72rem] font-bold text-muted-foreground">
                                <button type="button"
                                    class="inline-flex items-center gap-1.5 transition hover:text-foreground"
                    ...
                    <tbody>
                        <tr v-for="(expense, index) in expenses" :key="expense.id"
                            class="transition-colors hover:bg-primary/[0.03] text-center">
                            <td class="border-b text-center border-border/60 px-3 py-3 text-sm font-mono tabular-nums"
                                data-label="الرقم">
                                {{ visibleFrom + index }}
                            </td>
                            <td class="border-b text-center border-border/60 px-3 py-3 text-sm tabular-nums"
                                data-label="التاريخ">
                                {{ expense.expense_date ?? '-' }}
                            </td>
                            <td class="border-b text-center border-border/60 px-3 py-3" data-label="العنوان">
                            ...
                            <td class="border-b text-center border-border/60 px-3 py-3 text-sm font-mono font-semibold tabular-nums"
                                data-label="المبلغ">
                            ...
                            <td class="border-b text-center border-border/60 px-3 py-3" data-label="الحالة">
                                <span
                            ...
                            <td class="border-b text-center border-border/60 px-3 py-3 text-sm"
                                data-label="أضيف بواسطة">
                                {{ expense.creator?.name ?? expense.user?.name ?? '-' }}
                            </td>
                            <td class="border-b border-border/60 px-3 py-3 text-center align-middle"
                                data-label="الإجراءات">
                                <div class="flex justify-center items-center flex-wrap gap-1.5">
                                    <Button v-if="canView" type="button" variant="neumorphic" size="sm"
                                        class="h-8 rounded-lg px-2.5 text-xs" @click="emit('open-view', expense)">
                                        <Eye class="size-3.5" />
                                        عرض
                                    </Button>

                                    <Button v-if="canUpdate" type="button" variant="clay" size="sm"
                                        class="h-8 rounded-lg px-2.5 text-xs" @click="emit('open-edit', expense)">
                                        <Pencil class="size-3.5" />
                                        تعديل
                                    </Button>

                                    <Button v-if="canDelete" type="button" variant="destructive" size="sm"
                                        class="h-8 rounded-lg px-2.5 text-xs" @click="emit('delete-expense', expense)">
                        ...
                        <tr v-if="expenses.length === 0" class="table-empty-state">
                            <td colspan="9" class="px-3 py-14 text-center text-muted-foreground">
        ...
        <div
            class="flex flex-col gap-3 rounded-2xl border border-border/70 bg-secondary/20 px-4 py-3 mx-5 mb-5 sm:flex-row sm:items-center sm:justify-between">
            <p class="text-[0.72rem] text-muted-foreground">
                عرض {{ visibleFrom }}-{{ visibleTo }} من {{ total }}
            </p>
            <div class="flex items-center gap-2">
                <Button type="button" variant="outline" size="sm" class="h-8 rounded-lg px-3 text-[0.72rem]"
                    :disabled="localPage === 1" @click="emit('change-page', localPage - 1)">
                    السابق
                </Button>
                ...
                <Button type="button" variant="outline" size="sm" class="h-8 rounded-lg px-3 text-[0.72rem]"
                    :disabled="localPage >= totalPages" @click="emit('change-page', localPage + 1)">
                    التالي
                </Button>
            </div>
        </div>
