...
import RadiologyOrderController from '@/actions/App/Http/Controllers/Radiology/RadiologyOrderController';
...
type SortField = 'study_name' | 'modality' | 'status' | 'ordered_at' | 'created_at';
type SortDirection = 'asc' | 'desc';

const { orders, filters } = defineProps<{
    orders: PaginatedResponse<RadiologyOrder>;
    filters: {
        search: string | null;
        per_page: number;
...
defineOptions({
    layout: {
        breadcrumbs: [
            { title: 'طلبات الأشعة', href: RadiologyOrderController.index() },
        ],
    },
});
...
const statusClass = (status: string): string => {
    const map: Record<string, string> = {
        ...
        completed: 'border-success-300/70 bg-success-50 text-success-800 dark:border-success-500/40 dark:bg-success-500/15 dark:text-success-100',
        reported: 'border-info-300/70 bg-info-50 text-info-800 dark:border-info-500/40 dark:bg-info-500/15 dark:text-info-100',
...
const reload = (overrides: Record<string, any> = {}) => {
    router.get(RadiologyOrderController.index.url(), {
...
const heroMetrics = computed(() => [
    { label: 'إجمالي الطلبات', value: String(orders.meta.total), hint: 'جميع طلبات الأشعة' },
    { label: 'مرئي', value: String(visibleOrders.value.length), hint: 'الصفحة الحالية' },
]);
</script>

<template>
    <Head title="طلبات الأشعة" />

    <div class="mx-auto w-full max-w-[1680px] space-y-5 p-4 md:p-6">
        <InternalPageHero
            kicker="طلبات تصوير"
            title="طلبات الأشعة"
            description="إدارة دراسات التصوير والأساليب وطلبات الفحص للمريض."
            :metrics="heroMetrics"
        />
            ...
            <div class="space-y-3 rounded-2xl border border-border/70 bg-background/60 p-4">
                <div class="grid gap-3 md:grid-cols-[repeat(2,minmax(0,1fr))] md:items-end">
                    <div class="grid gap-2">
                        ...
                        <FilterSearch id="orders_search" v-model="localSearch" placeholder="Study, patient, modality" />
                    </div>
            ...
            <div class="ui-table-shell mt-4">
                <table class="ui-table">
                    <thead>
                        <tr>
                            <th class="px-3 py-2">
                                <button type="button" class="inline-flex items-center gap-1.5 font-semibold transition hover:text-foreground" @click="toggleSort('study_name')">
                                    الدراسة <component :is="sortIconFor('study_name')" class="size-3.5" />
                                </button>
                            </th>
                            <th class="px-3 py-2">الكود</th>
                            <th class="px-3 py-2">
                                <button type="button" class="inline-flex items-center gap-1.5 font-semibold transition hover:text-foreground" @click="toggleSort('modality')">
                                    الأسلوب <component :is="sortIconFor('modality')" class="size-3.5" />
                                </button>
                            </th>
                    ...
                    <tbody>
                        <tr v-for="order in visibleOrders" :key="order.id" class="ui-table-row">
                            <td class="px-3 py-2 font-medium">{{ order.study_name }}</td>
                            <td class="px-3 py-2 font-mono text-xs">{{ order.study_code ?? '-' }}</td>
                            <td class="px-3 py-2">{{ order.modality ?? '-' }}</td>
                        ...
                        <tr v-if="visibleOrders.length === 0" class="table-empty-state">
                            <td colspan="6" class="px-3 py-10 text-center text-muted-foreground">لا توجد طلبات أشعة.</td>
                        </tr>
                    </tbody>
                </table>
            </div>
